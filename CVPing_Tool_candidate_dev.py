...
def update_host_entry(host, status=None, status_color=None):
    ...
    if host in previous_host_status and previous_host_status[host] != status:
        # Sort the results based on the "Host Status" column
        sort_column("Host Status")
...
def sort_column(column):
    global sort_order
    if sort_order[column] == "ascending":
        reverse = False
...
results_treeview.heading("Host", text="Host", command=lambda: sort_column("Host"))
results_treeview.heading("Host IP", text="Host IP", command=lambda: sort_column("Host IP"))
results_treeview.heading("Host Status", text="Host Status")
