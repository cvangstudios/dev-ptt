...
def extract_csv_data(serial_number, data):
    ...
    try:
        ...
        # Check for ConfigurationIntent - if null/missing, skip this device
        config_intent = data.get('ConfigurationIntent')
        if config_intent is None:
            print(f"⚠️  Processing stopped for {serial_number} - ConfigurationIntent is null/missing")
            print(f"   Skipping this device and moving to next...")
            return None
            
        if not isinstance(config_intent, dict):
            print(f"⚠️  Processing stopped for {serial_number} - ConfigurationIntent is not a valid object (type: {type(config_intent)})")
            print(f"   Skipping this device and moving to next...")
            return None
        ...
        # Example 2: ConfigurationIntent field extraction (your use case)
        # Now safe to access ConfigurationIntent since we've validated it exists
        """
