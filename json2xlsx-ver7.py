...
def check_configuration_intent(serial_numbers, base_url):
    """
    Pre-check all serial numbers to categorize ConfigurationIntent data.
    All serials will be processed - this just determines extraction method.
    ...
    """
    print("🔍 Pre-checking all serials to categorize ConfigurationIntent data...")
    print("="*60)
    
    full_extraction_serials = []  # Valid ConfigurationIntent - can extract nested data
    basic_extraction_serials = []  # Null/invalid ConfigurationIntent - extract basic data only
    ...
    for i, serial_number in enumerate(serial_numbers, 1):
        ...
        try:
            ...
            # Check ConfigurationIntent to determine extraction method
            config_intent = data.get('ConfigurationIntent')
            if config_intent is None:
                print("📋 (Null ConfigurationIntent - basic extraction)")
                basic_extraction_serials.append(serial_number)
            elif not isinstance(config_intent, dict):
                print(f"📋 (Invalid ConfigurationIntent - basic extraction)")
                basic_extraction_serials.append(serial_number)
            else:
            ...
            print(f"❌ (API Error: Status {e.returncode} - will retry during processing)")
            basic_extraction_serials.append(serial_number)  # Still attempt processing
        except json.JSONDecodeError as e:
            print("❌ (Invalid JSON - will retry during processing)")
            basic_extraction_serials.append(serial_number)  # Still attempt processing
        except Exception as e:
            print(f"❌ (Error: {str(e)[:50]}... - will retry during processing)")
            basic_extraction_serials.append(serial_number)  # Still attempt processing
    ...
    if basic_extraction_serials:
        print(f"\n📋 Serials requiring basic extraction (null/invalid ConfigurationIntent):")
        for serial in basic_extraction_serials:
            print(f"   • {serial}")
        print(f"\n📝 These {len(basic_extraction_serials)} serials will use basic field extraction.")
    
    if full_extraction_serials:
        print(f"\n✅ Serials for full extraction (valid ConfigurationIntent):")
        for serial in full_extraction_serials:
            print(f"   • {serial}")
        print(f"\n🔧 These {len(full_extraction_serials)} serials will use full ConfigurationIntent extraction.")
    
    print(f"\n🚀 ALL {len(serial_numbers)} serials will be processed for JSON and CSV extraction.")
    print("="*60)
...
def main():
    ...
    # Pre-check all serials to categorize ConfigurationIntent
    ...
    print(f"📊 Will use full extraction for {len(full_extraction_serials)} serials, basic extraction for {len(basic_extraction_serials)} serials\n")
    ...
    for i, serial_number in enumerate(serial_numbers, 1):
        ...
        if data is not None:
            ...
            if csv_row is not None:
                ...
                if serial_number in full_extraction_serials:
    ...
    if basic_extraction_serials:
        print(f"\n📋 Basic extraction serials (null/invalid ConfigurationIntent):")
        for serial in basic_extraction_serials:
            print(f"   • {serial}")
    print("="*50)
