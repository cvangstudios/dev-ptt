            ...
            try:
                ...
                with self.cred_lock:
                    if not self.credentials_updated:
                        ...
                        print(f"Retrying {host} with new credentials...")
                        ...
                        try:
                            ...
                            logger.error(f"Authentication still failed for {host} after retry: {retry_error}")
                            return None, None, None, None
                    else:
                        # Already tried new credentials, just fail this device
                        logger.debug(f"Skipping {host} - auth failed with updated credentials")
                        return None, None, None, None
