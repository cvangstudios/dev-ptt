...
def extract_csv_data(serial_number, data):
    ...
    try:
        ...
        # Fields as direct URLs (Excel will auto-create hyperlinks)
        status_value = data.get('status', '')
        if status_value:
            csv_row['status_portal'] = f'https://status-portal.com/device/{serial_number}'
        
        ip_value = data.get('ip_address', '')
        if ip_value:
            csv_row['ip_address'] = ip_value  # Regular field
            csv_row['web_interface'] = f'https://{ip_value}'  # Auto-hyperlink
        
        email_value = data.get('email', '')
        if email_value:
            csv_row['email'] = f'mailto:{email_value}'  # Auto-hyperlink
        
        phone_value = data.get('phone', '')
        if phone_value:
            csv_row['phone'] = f'tel:{phone_value}'  # Auto-hyperlink
        """
        ...
        # Nested fields as direct URLs
        model_value = device_info.get('model', '')
        if model_value:
            csv_row['device_model'] = model_value  # Display value
            csv_row['model_docs'] = f'https://support.vendor.com/model/{model_value}'  # Auto-hyperlink
        ...
        if mgmt_ip:
            ...
            csv_row['mgmt_interface'] = f'https://{mgmt_ip}'  # Auto-hyperlink
        """
        ...
        """
        # Create direct URLs (no formulas needed)
        asset_id = data.get('asset_id', '')
        if asset_id:
            csv_row['asset_id'] = asset_id  # Display value
            csv_row['asset_portal'] = f'https://assets.company.com/device/{asset_id}'  # Auto-hyperlink
        
        # Create monitoring dashboard link using multiple fields
        device_type = data.get('type', '')
        site_code = data.get('site_code', '')
        if device_type and site_code:
            csv_row['monitoring_dashboard'] = f'https://monitor.com/site/{site_code}/type/{device_type}'
        
        # Create support ticket link with pre-filled info
        model = data.get('model', '')
        if model:
            csv_row['create_support_ticket'] = f'https://support.com/new?serial={serial_number}&model={model}'
        """
        ...
        for row_idx, data_row in enumerate(all_csv_data, 2):  # Start from row 2
            for col_idx, header in enumerate(headers, 1):
                ...
                if isinstance(value, str):
                    # Check if it's a URL that should be a hyperlink
