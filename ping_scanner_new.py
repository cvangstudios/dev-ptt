...
Examples:
  ...
  python ping_scanner.py -d local.lan -f servers.txt  # Scan with domain and custom file
    ...
    print(f"DEBUG: ping_host exists: {hasattr(scanner, 'ping_host')}")
    print(f"DEBUG: Class methods: {[m for m in dir(scanner) if not m.startswith('_') and callable(getattr(scanner, m))]}")
