...
def main():
    ...
    try:
        ...
        vlan_output = utilities.get_output(tab, "show vlan brief", "#", timeout=30)

        # Parse the VLAN output using TextFSM
        template_file = os.path.join(script_dir, "textfsm_templates", "cisco_ios_show_vlan.template")
        ...
        csv_filename = utilities.create_output_filename(tab, "vlan-info", "csv")
        ...
        with open(csv_filename, 'w', newline='') as csvfile:
            ...
            for vlan in parsed_vlans:
                writer.writerow({
                    ...
                    'INTERFACES': ', '.join(vlan.get('INTERFACES', []))
                })

        crt.Dialog.MessageBox(f"VLAN information has been exported to {csv_filename}")
