...
def main():
    ...
    try:
        ...
        switchport_output = utilities.get_output(tab, "show interface status", "#", timeout=30)

        # Parse the switchport output using TextFSM
        template_file = os.path.join(script_dir, "textfsm_templates", "cisco_ios_show_interfaces_status.template")
        ...
        csv_filename = utilities.create_output_filename(tab, "switchport-mapping", "csv")

        # Write parsed data to CSV
        with open(csv_filename, 'w', newline='') as csvfile:
            fieldnames = ['INTERFACE', 'STATUS', 'VLAN', 'DUPLEX', 'SPEED', 'TYPE']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            for port in parsed_switchports:
                writer.writerow({
                    'INTERFACE': port.get('PORT', ''),
        ...
        crt.Dialog.MessageBox(f"Switchport mapping has been exported to {csv_filename}")
